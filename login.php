	...
	if(ISSET($_POST['submit'])) {
		...
		if($bind) {
			...
			if($data["count"] == 1) {
				...
				/* Neccesary to bindParam so we can check if the username is in our table (palmi_licensing.users) or not */
				$statement->bindParam(1, $inputUserName);
				
				$statement->execute();
				/* fetch the statement in rows so we can grab the data and put it into sessions */
				$row = $statement->fetch();
				...
				if($statement->rowCount() == 1) { 
					...
					alertDanger("You are not authenticated to use this system. Please contact IT.");
				}
			} else {
